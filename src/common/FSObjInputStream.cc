...
void FSObjInputStream::readObj(int w, int slicesize, int remainsize) {
  struct timeval time1, time2;
  gettimeofday(&time1, NULL);
  int wi = 0, readsize;
  while(true) {
    int hasread = 0;
    readsize = ((wi % w == 0) ? slicesize + remainsize : slicesize);
    char* buf = (char*)calloc(readsize+4, sizeof(char));
    if (!buf) {
      cout << "FSObjInputStream::readObj.malloc buffer fail" << endl;
      return;
    }
    while(hasread < readsize) {
      int len = _underfs->readFile(_underfile, buf+4+hasread, readsize-hasread);
      if (len == 0) break;
      hasread += len;
    }
